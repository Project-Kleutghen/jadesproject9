...
export class AVComponentUtils {
    ...
    static getSpatialComponent(target) {
        const spatial = /xywh=([^&]+)/g.exec(target);
        let xywh = null;
    ...
    static getFirstTargetedCanvasId(range) {
        let canvasId;
    ...
    static retargetTemporalComponent(canvases, target) {
        let t = Utils.getTemporalComponent(target);
        if (t) {
            let offset = 0;
            let targetWithoutTemporal = target.substr(0, target.indexOf("#"));
    ...
    static formatTime(aNumber) {
        let hours, minutes, seconds, hourValue;
    ...
    static isHLSFormat(format) {
        return this.hlsMimeTypes.includes(format.toString());
    }
    static isMpegDashFormat(format) {
        return format.toString() === "application/dash+xml";
    }
    static canPlayHls() {
        const doc = typeof document === "object" && document, videoelem = doc && doc.createElement("video"), isvideosupport = Boolean(videoelem && videoelem.canPlayType);
