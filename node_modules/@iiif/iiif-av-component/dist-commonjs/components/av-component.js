...
const canvas_instance_events_1 = require("../events/canvas-instance-events");
const base_component_1 = require("@iiif/base-component");
const volume_events_1 = require("../events/volume-events");
const virtual_canvas_1 = require("../elements/virtual-canvas");
const canvas_instance_1 = require("./canvas-instance");
const diff_data_1 = require("../helpers/diff-data");
const get_first_targeted_canvas_id_1 = require("../helpers/get-first-targeted-canvas-id");
const av_component_events_1 = require("../events/av-component-events");
const logger_1 = require("../helpers/logger");
require("dashjs/dist/dash.mediaplayer.min");
require("../css/styles.less");
class AVComponent extends base_component_1.BaseComponent {
    constructor(options) {
        super(options);
    ...
    getCurrentCanvasInstance() {
        ...
        // @ts-ignore
        const canvasId = (0, get_first_targeted_canvas_id_1.getFirstTargetedCanvasId)(range);
        // @ts-ignore
    ...
    set(data) {
        ...
        const diff = (0, diff_data_1.diffData)(oldData, this._data);
        ...
        if (diff.includes('range') && this._data.range) {
            ...
            if (!range) {
                ...
                let canvasId = (0, get_first_targeted_canvas_id_1.getFirstTargetedCanvasId)(range);
                if (canvasId) {
    ...
    _reset() {
        ...
        if (this._data && this._data.helper && this._data.helper.manifest) {
            ...
            if (behavior && behavior.toString() === 'auto-advance') {
                ...
                const virtualCanvas = new virtual_canvas_1.VirtualCanvas();
    ...
    _initCanvas(canvas) {
        ...
        const canvasInstance = new canvas_instance_1.CanvasInstance({
        ...
        canvasInstance.on('play', () => {
            this.fire(av_component_events_1.Events.PLAY, canvasInstance);
        }, false);
        canvasInstance.on('pause', () => {
            this.fire(av_component_events_1.Events.PAUSE, canvasInstance);
        }, false);
        canvasInstance.on(av_component_events_1.Events.MEDIA_READY, () => {
            this._readyMedia++;
            canvasInstance.loaded();
        }, false);
        canvasInstance.on(av_component_events_1.Events.MEDIA_ERROR, (err) => {
            this.fire(av_component_events_1.Events.MEDIA_ERROR, err);
        }, false);
        canvasInstance.on(av_component_events_1.Events.WAVEFORM_READY, () => {
            this._readyWaveforms++;
        }, false);
        // canvasInstance.on(Events.RESETCANVAS, () => {
        //     this.playCanvas(canvasInstance.canvas.id);
        // }, false);
        canvasInstance.on(canvas_instance_events_1.CanvasInstanceEvents.PREVIOUS_RANGE, () => {
            this._prevRange();
            this.play();
        }, false);
        canvasInstance.on(canvas_instance_events_1.CanvasInstanceEvents.NEXT_RANGE, () => {
            this._nextRange();
            this.play();
        }, false);
        canvasInstance.on(av_component_events_1.Events.RANGE_CHANGED, (rangeId) => {
            this.fire(av_component_events_1.Events.RANGE_CHANGED, rangeId);
        }, false);
        canvasInstance.on(volume_events_1.VolumeEvents.VOLUME_CHANGED, (volume) => {
            this._setCanvasInstanceVolumes(volume);
            this.fire(volume_events_1.VolumeEvents.VOLUME_CHANGED, volume);
        }, false);
    }
    getCurrentRange() {
    ...
    _getCanvasInstanceById(canvasId) {
        ...
        if (this._data.virtualCanvasEnabled) {
            ...
            for (let i = 0; i < this.canvasInstances.length; i++) {
                ...
                if (id) {
                    const canvasInstanceId = manifesto_js_1.Utils.normaliseUrl(id);
