  ...
  - Current time: ${this.getTime()}
    ...
    validateExternalTime(time) {
        ...
        if (nextStop && currentStop !== nextStop) {
            logger_1.Logger.log('Skipping to next stop', { nextStop, currentStop });
    ...
    setRangeTime(rangeId, time, external = false) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (plan) {
                ...
                let newTime = (0, relative_time_1.timelineTime)(plan.start + time);
                if (external) {
    ...
    setRange(id) {
        logger_1.Logger.group('setRange', id);
        if (id === this.currentRange) {
            logger_1.Logger.log('id === this.currentRange');
            return this.getTime();
        }
        this.currentRange = id;
        if (id === this.currentStop.rangeId) {
            logger_1.Logger.log('id === this.currentStop.rangeId');
            // Or the start of the range?
            return this.getTime();
        }
        logger_1.Logger.log('Looking for stop', this.plan.stops);
        for (const stop of this.plan.stops) {
            if (stop.rangeId === id) {
                logger_1.Logger.log('Found stop, setting internalTime', stop.start);
        ...
        for (const stop of this.plan.stops) {
            if (stop.rangeStack.indexOf(id) !== -1) {
                logger_1.Logger.log('Found stop in rangeStack, setting internalTime', stop.start);
                this.setInternalTime(stop.start);
                this.advanceToStop(this.currentStop, stop, id, undefined, !this.playing);
                break;
            }
