...
var Helper = /** @class */ (function () {
    function Helper(options) {
        ...
        if (this.options.canvasId) {
            var canvasIndex = this.getCanvasIndexById(this.options.canvasId);
            if (canvasIndex !== null) {
    ...
    Helper.prototype.getAllRequiredStatements = function () {
        var statements = [];
        var range = this.getCurrentRange();
        if (range) {
            // @todo remove - BC for manifesto.
            var rangeStatement = range.getRequiredStatement
                ? range.getRequiredStatement()
                : null;
            if (rangeStatement) {
                statements.push(__assign(__assign({}, this.parseStatement(rangeStatement)), { source: "Range" }));
            }
        }
        var canvas = this.getCurrentCanvas();
        if (canvas) {
            // @todo remove - BC for manifesto.
            var canvasStatement = canvas.getRequiredStatement
                ? canvas.getRequiredStatement()
                : null;
            if (canvasStatement) {
                statements.push(__assign(__assign({}, this.parseStatement(canvasStatement)), { source: "Canvas" }));
            }
        }
        if (this.manifest) {
            var manifestStatement = this.manifest.getRequiredStatement();
            if (manifestStatement) {
                if (manifestStatement) {
                    statements.push(__assign(__assign({}, this.parseStatement(manifestStatement)), { source: "Manifest" }));
                }
            }
        }
        return statements;
    };
    Helper.prototype.getMostSpecificRequiredStatement = function () {
        var all = this.getAllRequiredStatements();
        var range = all.find(function (statement) { return statement.source === "Range"; });
        if (range) {
            return range;
        }
        var canvas = all.find(function (statement) { return statement.source === "Canvas"; });
        if (canvas) {
            return canvas;
        }
        var manifest = all.find(function (statement) { return statement.source === "Manifest"; });
        if (manifest) {
            return manifest;
        }
    ...
    Helper.prototype.parseStatement = function (statement) {
        return {
            label: statement.label ? statement.getLabel() : "",
            value: statement.value && statement.value.length ? statement.getValue() : ""
        };
    };
    ...
    Helper.prototype.getAccompanyingCanvas = function () {
        if (!this.manifest) {
            throw new Error(Errors_1.Errors.manifestNotLoaded);
        }
    ...
    Helper.prototype.getAccompanyingCanvasImage = function () {
        var accompanyingCanvas = this.getAccompanyingCanvas();
        if (accompanyingCanvas) {
            var content = accompanyingCanvas.getContent();
            if (content && content.length) {
                var annotation = content[0];
                var body = annotation.getBody();
                return body[0].id;
